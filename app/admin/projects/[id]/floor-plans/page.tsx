...
export default async function FloorPlansPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const plans = await getFloorPlansByProject(id).catch(() => [])
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex gap-2 mb-6">
            {[
              ...
              { href: `/admin/projects/${id}/floor-plans`, label: '户型管理', active: true },
          ...
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold text-gray-900">户型管理</h1>
            <Link href={`/admin/projects/${id}/floor-plans/new`}
          ...
          <FloorPlanList plans={plans} projectId={id} />
        </div>
      </main>
    </div>
  )
